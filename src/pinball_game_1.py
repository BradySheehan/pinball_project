...
class Table():
    ...
    def load_models(self):
        ...
        # self.ball_body = self.setup_ball_physics(self.ball, 50, 0.1)

        self.ball.set_pos(4.3, 2.85, 0.12)
        self.ball_mass = OdeMass()
        self.ball_mass.setSphere(50, .1)
        ...
        self.ball_geom = OdeSphereGeom(self.space, .1)
        # self.ball_geom.setCollideBits(BitMask32(0x00000002))
        # self.ball_geom.setCategoryBits(BitMask32(0x00000001))
        # self.ball_geom.setCollideBits(BitMask32(0x00000001))
        # self.ball_geom.setCategoryBits(BitMask32(0x00000002))
    ...
    def setup_ball_physics(self, ball, radius, mass):
        print "\t \t setup ball physics"
        ball.setPos(0, 0, 20)
        ball_mass = OdeMass()
        ball_mass.setSphere(mass, radius)
        ball_body = OdeBody(self.world)
        ball_body.setMass(ball_mass)
        ball_body.setPosition(ball.getPos(render))
        ball_body.setQuaternion(ball.getQuat(render))
        ball_geom = OdeSphereGeom(self.space, radius)
        # ball_geom.setCollideBits(BitMask32(0x00000002))
        # ball_geom.setCategoryBits(BitMask32(0x00000001))
        # ball_geom.setCollideBits(BitMask32(0x00000001))
        # ball_geom.setCategoryBits(BitMask32(0x00000002))
    ...
    def simulationTask(self, task):
        ...
        self.ball.setPosQuat(
            render, self.ball_body.getPosition(), Quat(self.ball_body.getQuaternion()))
        # ball_body.set_force(0,176.689,-1000)
        # ball_body.set_force(0,0, -100)
        self.ball_body.setForce(-1, -1, 0)
        # ball_body.setForce(1, 1, 0)

        base.camera.lookAt(self.ball_body.getPosition())
        self.contactgroup.empty()  # Clear the contact joints
        return task.cont

    def launch_ball(self):
        # pass
        taskMgr.doMethodLater(1.5, self.simulationTask, "Physics Simulation")
...
class Game():
    ...
    # def launch_ball(self):
    # 	# pass
    # 	taskMgr.doMethodLater(1.5, self.table.simulationTask, "Physics Simulation")
    # 	# taskMgr.remove('Physics Simulation')
